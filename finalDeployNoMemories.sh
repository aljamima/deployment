...
total=0
container=3
for rack in {1..3};                                      #--- num of racks to loop thru -- EXAMPLE: {start..end} ---#
do
  rackTotal=0
  ...
  do
	...
	do
		let "rackTotal+=1"
		let "total+=1"     
		#if [ "$shelf" -eq 5 ] && [ "$column" -gt 2 ]     # this means it wont do any miners past shelf 5 - position 4. so only 24 rigs that rack
		#then 
		...
		#pause prolly dont need thisone???
		echo "Scanning For New Devices"
		echo -e "\e[41;38;5;82m SCANNING FOR NEW DEVICE AT $position, PLEASE WAIT... \e[0m"
...
#		sort out2.txt > out2.sorted
		...
		ping -c 1 -w 0.2 $foundIp2
		echo "foundIP:$foundIp FoundIp2:$foundIp2"
		mac=$(timeout 1 ./macFromIp.sh $foundIp) ## it was running a seperate shell script in working version.
		echo "Miner was $foundIp, changing to $ipVar"
		#echo "$foundIp2" >> out1.txt && sort -t . -k 3,3n -k 4,4n out1.txt > out1.sorted
